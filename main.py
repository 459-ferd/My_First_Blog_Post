...
from forms import CreatePostForm, RegisterForm, UserLoginForm, CommentForm
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=form.email.data).first():
            flash("You've already signed in with that email, please login")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash('Please sign up or login to comment')
            return redirect(url_for('login'))
        new_comment = Comment(
